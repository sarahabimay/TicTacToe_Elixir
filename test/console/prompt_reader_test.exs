defmodule PromptReaderTest do
  ...
  test "it receives valid board size choice" do
    user_input = "1"
    expected_size = 3
    action_fn = fn -> assert PromptReader.request_board_size() == expected_size end
  ...
  test "it receives valid game_type choice" do
    user_input = "1"
    expected = "HVH"
    action_fn = fn -> assert PromptReader.request_game_type() == expected end
    IOAssert.assert_with_input(user_input, action_fn)
  end
  ...
  test "it receives invalid game_type choice" do
    ...
    user_input = "#{invalid_choice}\n#{valid_choice}"
    expected_type = "HVH"
    action_fn = fn -> assert PromptReader.request_game_type() == expected_type end
    IOAssert.assert_with_input(user_input, action_fn)
  end
  ...
  test "receives valid next move" do
    ...
    next_move = "5"
    action_fn = fn -> assert PromptReader.request_next_move(board) == 5 end
  ...
  test "receives invalid move" do
    ...
    board = Board.empty_board(dimension)
    moves = "0\n1"
    action_fn = fn -> assert PromptReader.request_next_move(board) == 1 end
  ...
  test "valid replay game choice" do
    yes_input = "1"
    expected = 1
    action_fn = fn -> assert PromptReader.play_again_option() == expected end
  ...
  test "keep requesting replay choice until valid" do
    input = "3\na\n0\n1\n"
    action_fn = fn -> assert PromptReader.play_again_option() == 1 end
